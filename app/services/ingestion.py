...
class IngestionService:
    ...
    def __init__(self) -> None:
        ...
        # Initialize YouTube client lazily to handle missing API keys
        self._youtube = None

    def _get_youtube_client(self):
        """Get YouTube client, initializing it lazily if needed."""
        if self._youtube is None:
            try:
                if not settings.youtube_api_key or settings.youtube_api_key == "your_youtube_api_key_here":
                    logger.info("YouTube API key not configured, YouTube ingestion will be skipped")
                    return None
    ...
    async def _ingest_youtube(self, products: List[str]) -> None:
        ...
        if youtube is None:
            logger.info("YouTube client not available, skipping YouTube ingestion")
            return
